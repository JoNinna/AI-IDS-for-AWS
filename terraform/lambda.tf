...
resource "aws_lambda_permission" "allow_s3_inference" {
  statement_id  = "AllowS3InvokeInference"
  action        = "lambda:InvokeFunction"
...
resource "aws_s3_bucket_notification" "combined_trigger" {
  ...
  lambda_function {
    ...
    events              = ["s3:ObjectCreated:*"]
    filter_prefix       = "processed/suricata/"
    filter_suffix       = ".json"
  }
  ...
  lambda_function {
    lambda_function_arn = aws_lambda_function.ai_hids_inference.arn
    events              = ["s3:ObjectCreated:*"]
    filter_prefix       = "processed/falco/"
    filter_suffix       = ".json"
  }

  depends_on = [
    aws_lambda_permission.allow_s3_etl,
    aws_lambda_permission.allow_s3_inference
  ]
}
