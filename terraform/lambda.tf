...
resource "aws_lambda_function" "ai_hids_inference" {
  ...
  filename         = "${path.module}/../dist/lambda_inference.zip"
  source_code_hash = filebase64sha256("${path.module}/../dist/lambda_inference.zip")
...
resource "aws_lambda_permission" "hids-inference" {
  statement_id  = "AllowS3Invoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.ai_hids_inference.function_name
  principal     = "s3.amazonaws.com"
  source_arn    = aws_s3_bucket.log_bucket.arn
}
